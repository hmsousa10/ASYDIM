...
if (empty($formation)) { header("location: index.php"); exit(); }
...
<head>
    ...
    <title>ASYDIM Admin — Editar Formação</title>
...
<script>document.getElementById('adm-page-title').textContent = 'Editar Formação';</script>

<div class="adm-page-header">
    <div>
        <h1>Editar Formação</h1>
        <p style="color:var(--text-muted);margin:4px 0 0;font-size:0.9rem;"><?php echo htmlspecialchars($formation['name']); ?></p>
    </div>
    <a href="index.php" class="btn-outline"><i class="ph ph-arrow-left"></i> Voltar</a>
</div>

<form id="formationForm" action="../includes/formations/edit.php?id=<?php echo $id; ?>" method="POST" enctype="multipart/form-data">
<div class="adm-form-card">
    <div class="form-grid form-grid-3">
        <div class="form-group form-full">
            <label class="form-label" for="name">Nome da Formação</label>
            <input class="form-input" type="text" name="name" id="name" value="<?php echo htmlspecialchars($formation['name']); ?>" required>
        </div>
        <div class="form-group">
            <label class="form-label" for="slug">Abreviatura</label>
            <input class="form-input" type="text" name="slug" id="slug" value="<?php echo htmlspecialchars($formation['slug']); ?>" required>
        </div>
        <div class="form-group">
            <label class="form-label" for="duration">Duração (horas)</label>
            <input class="form-input" type="number" min="1" name="duration" id="duration" value="<?php echo $formation['duration']; ?>" required>
        </div>
        <div class="form-group">
            <label class="form-label" for="unit_price">Preço (€)</label>
            <input class="form-input" type="number" min="0" step="0.01" name="unit_price" id="unit_price" value="<?php echo number_format($formation['unit_price'] / 100, 2, '.', ''); ?>" required>
        </div>
        <div class="form-group form-full">
            <label class="form-label" for="description">Descrição</label>
            <textarea class="form-textarea" name="description" id="description" rows="5" required><?php echo htmlspecialchars($formation['description']); ?></textarea>
        </div>
        <div class="form-group">
            <label class="form-label" for="beginning_date">Data de Início</label>
            <input class="form-input" type="date" name="beginning_date" id="beginning_date" value="<?php echo $formation['beginning_date']; ?>" required>
        </div>
        <div class="form-group">
            <label class="form-label" for="ending_date">Data de Fim <span style="color:var(--text-muted);font-weight:400;">(opcional)</span></label>
            <input class="form-input" type="date" name="ending_date" id="ending_date" value="<?php echo $formation['ending_date'] ?? ''; ?>">
        </div>
        <div class="form-group">
            <label class="form-label" for="category">Categoria</label>
            <select class="form-select" name="category" id="category">
                <?php foreach ($categories as $cat): ?>
                    <option value="<?php echo htmlspecialchars($cat['slug']); ?>" <?php echo ($cat['slug'] === $formation['category']) ? 'selected' : ''; ?>>
        ...
        <div class="form-group form-full">
            ...
            <img src="<?php echo $formation['image']; ?>" alt="Imagem atual" style="height:160px;width:auto;border-radius:var(--radius);object-fit:cover;border:1px solid var(--border);">
        </div>
        <?php endif; ?>

        <div class="form-group form-full">
            <label class="form-label" for="image">Nova Imagem <span style="color:var(--text-muted);font-weight:400;">(opcional)</span></label>
            <input class="form-input" type="file" name="image" id="image" accept="image/jpeg,image/png,image/webp" style="cursor:pointer;">
