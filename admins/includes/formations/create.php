...
if (empty($name) || empty($slug) || $duration <= 0 || empty($description) || empty($beginning_date) || empty($category)) {
    header("location: " . $arrConfig["url_admin"] . "/formations/create.php?error=3");
    exit();
}
...
if ($beginning_ts === false) {
    header("location: " . $arrConfig["url_admin"] . "/formations/create.php?error=1");
    exit();
}
...
if (!isset($_FILES['image']) || $_FILES['image']['error'] !== UPLOAD_ERR_OK) {
    header("location: " . $arrConfig["url_admin"] . "/formations/create.php?error=2");
    exit();
}
...
$result = my_query(
    "INSERT INTO formations (name, description, slug, beginning_date, ending_date, duration, category, image, unit_price)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
    [$name, $description, $slug, $beginning_date, $end_ts ? $end_date : null, $duration, $category, $image_url, $unit_price]
);

if (!$result) {
    header("location: " . $arrConfig["url_admin"] . "/formations/create.php?error=4");
    exit();
}
